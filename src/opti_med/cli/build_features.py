"""Command-line entry point for building the processed MVP feature cohort."""
...
from opti_med.features.builder import MinimalFeatureBuilder, summarize_processed_cohort


def build_parser() -> argparse.ArgumentParser:
    """Create the CLI argument parser."""
    parser = argparse.ArgumentParser(
        description="Build the processed older-adult medication cohort with MVP features."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output CSV path for the processed cohort file.",
    )
    parser.add_argument(
        "--age-threshold",
        type=int,
...
def main() -> int:
    """Run the processed cohort build workflow."""
    ...
    builder = MinimalFeatureBuilder(settings)
    try:
    ...
    for summary in summarize_processed_cohort(result.dataframe):
        print(f"- {summary}")
    return 0
